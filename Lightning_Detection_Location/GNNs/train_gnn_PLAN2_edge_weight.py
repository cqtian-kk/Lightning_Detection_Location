...
from Trans_model_PLAN2_edge_attr import *
...
def read_args():
    ...
    parser.add_argument("--board_path", default='./loss_path/loss_PLAN2_edge_attr_/', help="board_path")
    parser.add_argument("--save_model_path",default='./model_path/mode_PLAN2_edge_attr_/', help="Save_model_path")
...
def main(args):
    ...
    # 文件夹路径
    folder_path = "dataNBEIBP/tt" 
    # train_data, val_data, train_label, val_label = readnpz(folder_path = folder_path, trainval_rate = 0.8)
    ...
    # train_mask = np.ones((train_data.shape[0],6), dtype=int)
    # val_mask = np.ones((val_data.shape[0],6), dtype=int)
    
    # train_mask = mask_array(train_data.shape[0])
    val_mask = mask_array(val_data.shape[0])
    np.savez(args.save_model_path+'/temp_traindata0210.npz',d1 = train_data, d2 = train_label)
    np.savez(args.save_model_path+'/temp_valdata0210.npz',d1 = val_data, d2 = val_label)
    np.savez(args.save_model_path+'/val_mask.npz',d1 = val_mask)
    # train_mask = np.ones((train_data.shape[0],6), dtype=int)
    # val_mask = np.ones((val_data.shape[0],6), dtype=int)
    # train_mask[train_data.shape[0]//2:,:] = mask_array(train_data.shape[0]//2)
    # val_mask[val_data.shape[0]//2:,:] = mask_array(val_data.shape[0]//2)
    # edge_index = np.zeros([36,2])
    # k = 0
    # for i in range(6):
    #     for j in range(6):
    #         edge_index[k,0] = i
    ...
    # train_dataset = MyGNNDataset_norandmask(train_data, train_label,train_mask)
    val_dataset = MyGNNDataset_norandmask(val_data, val_label, val_mask)
    # train_dataset = MyGNNDataset(train_data, train_label, edge_index.T)
    # val_dataset = MyGNNDataset(val_data, val_label, edge_index.T) 
    ...
    for epoch in range(epoch_total):
        ...
        for mydata in train_loader:
            ...
            # loss = criterion(output[:,0:2], mydata.y[:,0:2])+40.5/14.5*criterion(output[:,2], mydata.y[:,2])
            loss.backward()
            loss_all += loss.item()           
            optimizer.step()
        ...
        for data in val_loader:
            ...
            # loss_test = criterion(output[:,0:2], data.y[:,0:2])+40.5/14.5*criterion(output[:,2], data.y[:,2])
