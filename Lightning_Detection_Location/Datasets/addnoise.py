...
for file_name in os.listdir(folder_path):
    if file_name.endswith(".npz"):
        ...
        for m in tqdm(range(batch)):
            random_array = np.random.uniform(snrs_max, snrs_min, size=(6,))
            noises = data_noises[m]
            dataforlig[m] = add_noises_with_snr(ligdata_clear[m], noises, random_array)
        np.savez('./dataset/'+file_name[:-4]+'_lowf'+str(lowfnoise_rand1)+'-'+
                 str(lowfnoise_rand2)+'_snrs'+str(snrs_min)+'_'+str(snrs_max)+'.npz',d1 = dataforlig, d2 = label_clear)
